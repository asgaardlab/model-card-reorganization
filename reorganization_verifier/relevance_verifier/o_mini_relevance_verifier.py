...
class OMiniRelevanceVerifier(BaseRelevanceVerifier):
    def get_save_path(self, model_id: str, next_iteration_no: int = None, sub_dir_name: str = None) -> Path:
        return super().get_save_path(model_id, next_iteration_no, 'o4_mini')


if __name__ == '__main__':
    o_mini_client = OpenAIReasoningJsonClient(constants.O4_MINI, constants.OPENAI_API_KEY)
    relevance_verifier = OMiniRelevanceVerifier(Path('common_prompt_template.md'), get_json_schema(), o_mini_client)

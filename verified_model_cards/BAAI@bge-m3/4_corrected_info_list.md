...
- <details>
  ...
  - The previous test results were lower due to mistakenly removing passages with the same id as the query from the search results.
  - After correcting this mistake, the overall performance of BGE-M3 on MIRACL is higher than previous results.
  - The experimental conclusion remains unchanged.
  - Other results are not affected by this mistake.
  - To reproduce the previous lower results, add the `--remove-query` parameter when using `pyserini.search.faiss` or `pyserini.search.lucene` to search the passages.
  </details>
